...
def construir_mapa(LISTA_PLATAFORMAS,LARGURA,TELA,ALTURA):
    dados = {'plataforma': [],'plataforma posicao':[],'itens':[],'itens posicao':[]}
    P_Y = ALTURA -30
    ...
    for item, item_aux, i in zip(itens, itens_aux, range(len(itens_aux))):
        if item.get_cor()!= cores_plataforma['cinza'] and item_aux is not None:
            if item.sumir_item(R_PLAYER, item_aux):
                itens_aux.pop(i)
                itens.pop(i) 
                return 'moeda' if item.get_cor() == cores_plataforma['dourado'] else 'diamante' if item.get_cor() == cores_plataforma['ciano'] else 'vida' if item.get_cor() == cores_plataforma['roxo'] else False
    return False        
...
while rodar:
    ...
    X_PLAYER =X_PLAYER - 10 if pygame.key.get_pressed()[K_a] else X_PLAYER
    X_PLAYER =X_PLAYER + 10 if pygame.key.get_pressed()[K_d] else X_PLAYER
    Y_PLAYER =Y_PLAYER - 10 if pygame.key.get_pressed()[K_w] else Y_PLAYER
    Y_PLAYER =Y_PLAYER + 10 if pygame.key.get_pressed()[K_s] else Y_PLAYER

    
    R_PLAYER = pygame.draw.rect(TELA, PLAYER, (X_PLAYER, Y_PLAYER, 20, 20))

    coletou = update_mapa(dados['plataforma'], dados['plataforma posicao'], dados['itens'], dados['itens posicao'], R_PLAYER)
    if coletou == 'moeda':
        moedas += 1
        pontuacao +=5
